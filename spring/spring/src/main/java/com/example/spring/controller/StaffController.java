...
public class StaffController {
    ...
    /**
     * REST endpoint to retrieve a staff member by name.
     *
     * @param staffName The name of the staff member to retrieve.
     * @return The staff member with the specified name and HTTP status code 200 (OK) if found.
     */
    @GetMapping("/name/{name}")
    public ResponseEntity<Staff> getStaffByName(@PathVariable("name") String staffName) {
        Staff staff = staffService.getStaffbyName(staffName);
        if (staff != null) {
            return new ResponseEntity<>(staff, HttpStatus.OK);
        } else {
    ...
    /**
     * REST endpoint for staff login.
     *
     * @param loginRequest The login request containing username and password.
     * @return A success message and HTTP status code 200 (OK) if authentication is successful,
     *         otherwise HTTP status code 401 (Unauthorized).
     */
    @PostMapping("/login")
    public ResponseEntity<String> login(@RequestBody LoginRequest loginRequest) {
        boolean isAuthenticated = staffService.authenticate(loginRequest.getUsername(), loginRequest.getPassword());
        if (isAuthenticated) {
     ...
     */
    @PutMapping("{id}")
    public ResponseEntity<Staff> updateStaff(@PathVariable("id") int staffId, @RequestBody Staff staff) {
        staff.setId(staffId);
